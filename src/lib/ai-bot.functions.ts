...
const upsertSchema = z.object({
  ...
  model_provider: z.enum(["lovable", "openai", "anthropic", "google"]).default("lovable"),
  model_name: z.string().min(1).max(120).default("google/gemini-3-flash-preview"),
  temperature: z.number().min(0).max(2).default(0.4),
  max_tokens: z.number().int().min(64).max(8000).default(1024),
  system_extra: z.string().max(4000).nullable().optional(),
  system_prompt_md: z.string().max(20000).nullable().optional(),
...
export const upsertBotConfig = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const payload = {
      ...
      system_extra: data.system_extra ?? null,
      system_prompt_md: data.system_prompt_md ?? null,
